...
async function clickLabel(name:string){const b=host.querySelector<HTMLButtonElement>(`button[aria-label="${name}"]`);expect(b,name).not.toBeNull();await act(async()=>b!.click());}
...
describe('complete CO learning UI',()=>{
 it('review regression: formerly guided-only topics expose the new graded bank and retain guided work',async()=>{const topic=topicStudy.topics.find(t=>t.id==='CO_T01_HISTORY')!;await act(async()=>root.render(<MemoryRouter><COStudyMode topic={topic} mode="practice"/></MemoryRouter>));await waitFor('.ds-lesson-block');expect(host.textContent).toContain('Unscored guided practice');expect(host.querySelectorAll('.ds-exercise-list-item')).toHaveLength(8);expect(host.textContent).not.toContain('Topic practice is not available yet');expect(host.textContent).not.toContain('No automatically graded exercises');});
 ...
 it.each(topicStudy.topics.filter(t=>t.subjectId==='CSE1400_CO'))('$id has a real lazy lesson, flashcards, practice or rubric/tool, with no mastery mutation',async topic=>{
  ...
  await render('flashcards');await waitFor('#card-prompt');const first=host.querySelector('#card-prompt')!.textContent;await clickLabel('Show the back of this flashcard');expect(host.querySelector<HTMLElement>('#card-answer')!.getAttribute('aria-hidden')).toBe('false');await click('Next →');expect(host.querySelector<HTMLElement>('#card-answer')!.getAttribute('aria-hidden')).toBe('true');await click('Shuffle');await click('Reset order');expect(host.querySelector('#card-prompt')!.textContent).toBe(first);
  await render('practice');await waitFor('.ds-exercise-list-item,.ds-lesson-block,.ds-tool-card');expect(host.textContent).not.toMatch(/\d+%|predicted grade/);expect(await repo.exportBackup()).toEqual(before);
 });
});
