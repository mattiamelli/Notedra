...
describe('Fixed authored Java predictions',()=>{
 it('retains the 33 independently reasoned IP items and all other definitions including eight Assembly traces',()=>{expect(items).toHaveLength(33);expect(allExercises.filter(e=>!e.id.startsWith('ds.practice.advanced-')&&!e.id.startsWith('ds.practice.p7-')&&!e.id.startsWith('ds.practice.interactive-')&&e.task.kind!=='ip-fixed')).toHaveLength(75);expect(oracles.map(o=>o.exerciseId)).toEqual(items.map(e=>e.id));});
