...
describe('launch legal and security surfaces',()=>{
  ...
  it('explains the real anonymous, account, sync, backup, cookie and analytics behavior',async()=>{
    ...
    for(const expected of ['need an account to enter the study app','Supabase Auth','synchronized learner snapshot','export a JSON backup','Anonymous product analytics are optional','PostHog in the European Union','random, pseudonymous identifier in localStorage','D1/D3/D7 retention','does not send study answers','not used for advertising tracking','withdraw consent at any time','removes only the PostHog analytics identifier','strictly necessary security or session cookies','does not sell learner data'])expect(text).toContain(expected);
