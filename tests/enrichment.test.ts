...
describe('supplemental authority and immutable authoring',()=>{
 ...
 it('retains all three historical grader fingerprints and 61 old item bindings',()=>{
  expect(allExercises.filter(e=>e.task.kind!=='enrichment-exact')).toHaveLength(61);
  ...
  for(const e of allExercises.filter(e=>e.task.kind!=='enrichment-exact'))expect(versionBinding(e)).toMatch(/^1:sha256:[a-f0-9]{64}:grader:[a-f0-9]{64}$/);
 });
});
