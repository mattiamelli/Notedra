...
describe('Sites HTTP delivery',()=>{
 ...
 it('keeps the canonical sitemap stable while adding initial lesson documents',()=>{expect(publicReleasePages()).toHaveLength(47);expect(publicDocumentPages()).toHaveLength(90);expect(new Set(publicDocumentPages().map(p=>p.path)).size).toBe(90);});
});
