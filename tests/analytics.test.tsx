...
describe('privacy-first analytics contract',()=>{
 it('registers the exact minimum product taxonomy',()=>expect(Object.keys(analyticsEventRegistry)).toEqual(['product_tour_started','product_tour_completed','product_tour_skipped','first_practice_started','first_practice_completed','practice_started','practice_completed','study_method_recommended','study_method_selected','study_path_generated','study_path_activity_opened','mistake_book_opened','review_started','exam_started','exam_completed','next_action_shown','next_action_opened','course_opened','topic_opened']));
 ...
 it('accepts only canonical Mistake Book context',()=>{expect(track('mistake_book_opened',{course_id:'CSE1400_CO',topic_id:'CO_T01_HISTORY',source_surface:'mistake_book'})).toBe(true);expect(track('review_started',{course_id:'CSE1400_CO',topic_id:'CO_T01_HISTORY',activity_type:'practice',source_surface:'mistake_book'})).toBe(true);const unsafe=track as (event:string,properties:Record<string,unknown>)=>boolean;expect(unsafe('review_started',{course_id:'CSE1400_CO',topic_id:'CO_T01_HISTORY',activity_type:'practice',source_surface:'mistake_book',answer:'private'})).toBe(false);expect(sink.records.map(record=>record.event)).toEqual(['mistake_book_opened','review_started']);});
