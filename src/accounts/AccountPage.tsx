...
export function AccountPage() {
  ...
  return <div className="ds-account"><header className="ds-page-heading"><p className="ds-eyebrow">Your study space</p><h1>Account &amp; cloud sync</h1><p>Manage your identity, appearance, sync, and local study data.</p></header>
    <div className="ds-account-grid">
      ...
      <section className="ds-account-card" aria-labelledby="appearance-heading"><p className="ds-eyebrow">Personalization</p><h2 id="appearance-heading">Appearance</h2><p>Choose a theme or let DelftStudy follow this device automatically.</p>
      ...
      <section className="ds-account-card" aria-labelledby="security-heading"><p className="ds-eyebrow">Access</p><h2 id="security-heading">Security</h2>
        {user?<><dl className="ds-security-list"><div><dt>Email</dt><dd>{user.email}</dd></div><div><dt>Password</dt><dd aria-label="Password hidden">••••••••</dd></div></dl><p>Your password is handled by Supabase Auth and is never stored or displayed by DelftStudy.</p><button className="ds-button ds-button-quiet" type="button" disabled={busy||!account.auth} onClick={()=>void action(()=>account.auth!.signOut())}>Sign out</button></>:account.config.status==='configured'?<form className="ds-auth-form" onSubmit={submit}><h3>{creating?'Create an account':'Sign in'}</h3>{creating&&<label>Display name<input required maxLength={80} autoComplete="name" value={displayName} onChange={e=>setDisplayName(e.target.value)}/></label>}<label>Email<input type="email" autoComplete="email" required value={email} onChange={e=>setEmail(e.target.value)}/></label><label>Password<input type="password" autoComplete={creating?'new-password':'current-password'} required minLength={creating?8:1} value={password} onChange={e=>setPassword(e.target.value)}/></label><div className="ds-form-actions"><button className="ds-button ds-button-primary" type="submit" disabled={busy||!account.auth}>{busy?'Please wait…':creating?'Create account':'Sign in'}</button><button className="ds-button ds-button-quiet" type="button" disabled={busy} onClick={()=>{setCreating(!creating);setPassword('');setError('');}}>{creating?'Use an existing account':'Create an account instead'}</button>{account.state.phase==='error'&&<button className="ds-button ds-button-quiet" type="button" disabled={busy} onClick={()=>void action(()=>account.auth!.signOut())}>Retry sign-out</button>}</div><p>Passwords are sent to Supabase Auth and are not stored by DelftStudy. If email verification is enabled, verify first and then sign in here.</p></form>:<p>Account access is unavailable in this environment. Anonymous local study remains available.</p>}
      </section>
