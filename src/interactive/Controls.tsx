...
export function MapView({map,task,answer,onChange,disabled=false}:{map:MapSpec;task?:Extract<InteractiveTask,{kind:'kmap-fill'}>;answer?:Answer;onChange?:(answer:Answer)=>void;disabled?:boolean}){
  ...
  return <div className="logic-map-scroll"><table className="logic-map"><caption>{t('interactive.kmapCaption',{rows:map.variables.slice(0,layout.rowBits).join(''),columns:map.variables.slice(layout.rowBits).join('')})}</caption><thead><tr><th scope="col">{map.variables.slice(0,layout.rowBits).join('')} / {map.variables.slice(layout.rowBits).join('')}</th>{layout.columns.map(column=><th scope="col" key={column}>{column}</th>)}</tr></thead><tbody>{layout.cells.map((row,rowIndex)=><tr key={rowIndex}><th scope="row">{layout.rows[rowIndex]}</th>{row.map(minterm=><td key={minterm}>{task&&!task.given.includes(minterm)?<select disabled={disabled} aria-label={t('interactive.mintermLabel',{assignment:layoutLabel(map,minterm),minterm})} value={tokens.find(token=>token.startsWith(`m${minterm}=`))?.split('=')[1]??''} onChange={event=>onChange?.({kind:'choice',value:[...tokens.filter(token=>!token.startsWith(`m${minterm}=`)),...(event.target.value?[`m${minterm}=${event.target.value}`]:[])]})}><option value="">—</option>{slotsFor(task).find(slot=>slot.id===`m${minterm}`)!.options.map(value=><option key={value}>{value}</option>)}</select>:<span aria-label={t('interactive.mintermValue',{assignment:layoutLabel(map,minterm),minterm,value:map.cells[minterm]})}>{map.cells[minterm]}{task&&<small>{t('interactive.given')}</small>}</span>}<small className="boolean-minterm" aria-label={mintermLiterals(map.variables,minterm).map(literalLabel).join(' AND ')}>{mintermLiterals(map.variables,minterm).map(token=><BooleanLiteral key={token} token={token}/>)}</small></td>)}</tr>)}</tbody></table></div>;
}
...
export default function InteractiveControls({task,answer,onChange,disabled}:{task:InteractiveTask;answer:Answer;onChange:(answer:Answer)=>void;disabled:boolean}){
  const {t,lt}=useI18n(),tokens=answer.kind==='choice'?answer.value:[];
...
export function InteractiveSpecification({task}:{task:InteractiveTask}){
  ...
  const map=task.map;
  return <p className="logic-specification">{task.specification.kind==='formula'?<>{t('interactive.function')}: <BooleanFormula formula={task.specification.formula}/></>:t('interactive.mapSpecification',{ones:map.cells.flatMap((value,index)=>value===1?[index]:[]).join(', ')||t('interactive.none'),cares:map.cells.flatMap((value,index)=>value==='X'?[index]:[]).join(', ')||t('interactive.none')})}</p>;
}
