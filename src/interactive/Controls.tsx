...
export default function InteractiveControls({task,answer,onChange,disabled}:{task:InteractiveTask;answer:Answer;onChange:(a:Answer)=>void;disabled:boolean}){
 ...
 function shapeLabel(node:FormulaShape):string {
  ...
  const op=typeof node.operator==='string'?node.operator:selected[node.operator.slot];
  return '('+shapeLabel(node.left)+' '+(op??'blank connective')+' '+shapeLabel(node.right)+')';
 }
 function shape(node:FormulaShape):ReactNode{
  if(node.kind==='atom')return selected[node.slot]?<BooleanLiteral token={selected[node.slot]}/>: '□';if(node.kind==='fixed')return node.name;
 ...
 return <fieldset className="logic-workspace" disabled={disabled} aria-describedby="answer-rules"><legend>{task.kind==='logic-build'?'Construct your formula':'Complete the map'}</legend>
  {task.kind==='logic-build'?<><div className="logic-preview" aria-label="Your formula" aria-live="polite">{shape(task.shape)}</div><div className="logic-slots">{task.slots.map(slot=><label key={slot.id}>{slot.label}<select aria-label={slot.label} value={selected[slot.id]??''} onChange={e=>onChange({kind:'choice',value:[...tokens.filter(t=>!t.startsWith(slot.id+'=')),...(e.target.value?[slot.id+'='+e.target.value]:[])]})}><option value="">Choose…</option>{slot.options.map(o=><option value={o} key={o} aria-label={symbols[o]??literalLabel(o)}>{symbols[o]??literalText(o)}</option>)}</select></label>)}</div></>:<MapView map={task.map} task={task} answer={answer} onChange={onChange} disabled={disabled}/>}
...
export function InteractiveSpecification({task}:{task:InteractiveTask}){
 ...
 const map=task.map;
 return <p className="logic-specification">{task.specification.kind==='formula'?<>Function: <BooleanFormula formula={task.specification.formula}/></>:`Output 1 at minterms: ${map.cells.flatMap((v,i)=>v===1?[i]:[]).join(', ')||'none'}. Don’t-cares: ${map.cells.flatMap((v,i)=>v==='X'?[i]:[]).join(', ')||'none'}. All other outputs: 0.`}</p>;
}
