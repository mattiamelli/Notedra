...
export function ProgressPage(){
 ...
 <label>{t('progress.topic')}<select value={selected?.id??''} onChange={e=>setParams({course:courseId,...(e.target.value?{topic:e.target.value}:{})})}><option value="">{t('progress.allTopics')}</option>{topics.map(topic=><option key={topic.id} value={topic.id}>{topic.name}</option>)}</select></label>
