...
export function ProgressPage(){
 ...
 <label>{t('progress.course')}<select value={courseId} onChange={e=>setParams({course:e.target.value})}>{courses.map(c=><option key={c.subject_id} value={c.subject_id}>{c.name}</option>)}</select></label>
 ...
 <section className="progress-card"><h2>{t('progress.next')}</h2><p>Strongest recorded skills: {result.courses.find(c=>c.id===courseId)!.strongest.join('; ')||t('dashboard.notEnoughPractice')}.</p><p>Recent mistakes to review: {result.courses.find(c=>c.id===courseId)!.review.join('; ')||'None recorded'}.</p><p>{result.courses.find(c=>c.id===courseId)!.missing.length} skills do not yet have enough reliable practice. This does not mean you performed poorly.</p><Link to={`/study-plan?course=${courseId}`}>{t('progress.openPath')} →</Link> · <Link to={`/mistakes?course=${courseId}`}>{t('progress.reviewMistakes')} →</Link> · <Link to={`/exams/${courseId}/setup`}>{t('progress.mockExam')} →</Link></section>
 <label>{t('progress.topic')}<select value={selected?.id??''} onChange={e=>setParams({course:courseId,...(e.target.value?{topic:e.target.value}:{})})}><option value="">{t('progress.allTopics')}</option>{topics.map(topic=><option key={topic.id} value={topic.id}>{lt(topic.name)}</option>)}</select></label>
