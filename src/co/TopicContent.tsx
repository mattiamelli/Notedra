...
export function TopicContent({topic,mode,content}:{content:COTopicContent}&COStudyProps){
 ...
 if(mode==='learn')return <>{tool?.id==='assembly'?<AssemblyToolCard/>:tool&&<p className="ds-co-tool-link"><Link className="ds-button" to={studyPath(topic,'practice')+'#co-workspace'}>{t('course.exploreTool',{tool:lt(tool.name)})}</Link></p>}<LearnMode topic={topic} content={content.lesson}/></>;
