...
export function explainAnswer(exercise:PracticeExercise,answer:Answer,result:GradeResult):Explanation|null {
 ...
 const profile=(profiles as FeedbackProfile[]).find(p=>p.exerciseId===exercise.id&&p.binding===versionBinding(exercise));
 if(!profile)return {why:result.correct?'Your response matches the independently checked reference.':'Your response differs from the reference. Compare your result with the steps below; no specific misconception is inferred.',reasoning:result.explanation,remember:exercise.rules,profileId:null,version:null,misconception:null,evidenceIds:[]};
 const key=answer.kind==='text'?patternKey(exercise,answer.value):null;
 const path=!result.correct&&key!==null?profile.paths.find(p=>patternKey(exercise,p.answer)===key):undefined;
 return {why:result.correct?'Your response matches the reference. The reasoning below explains why.':path?.why??profile.fallback,reasoning:profile.reasoning,remember:profile.remember,profileId:profile.id,version:profile.version,misconception:path?(misconceptions as Misconception[]).find(m=>m.id===path.misconceptionId)??null:null,evidenceIds:profile.evidenceIds};
}
