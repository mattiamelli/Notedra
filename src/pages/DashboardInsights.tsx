...
export default function DashboardInsights({children}:{children?:ReactNode}) {
  ...
  return <>
    <section className="ds-dash-panel ds-dash-plan"><div className="ds-section-heading"><h2><ShellIcon name="calendar"/>Your Study Path</h2><span>20 min · flexible plan</span></div>
      {!learning?.snapshot?<p role="status">{learning?.message??'Your saved study history is unavailable.'}</p>:path.length?<ol className="ds-dashboard-plan">{path.map((action,i)=><li key={action.id}><span className="ds-plan-number">{i+1}</span><div><h3>{action.title}</h3><p>{action.reason}</p><small>{action.minutes} min · {action.durationSource}</small></div><Link className="ds-button" to={action.to}>Open</Link></li>)}</ol>:<div className="ds-plan-empty"><span className="ds-plan-number"><ShellIcon name="book" size={28}/></span><div><h3>Start with a practice topic</h3><p>Complete a few exercises to receive suggestions based on the skills that need attention.</p><Link className="ds-button" to="/practice">Start Practice <ShellIcon name="arrow" size={16}/></Link></div></div>}
    ...
    {children}
    <section className="ds-dash-panel ds-dash-mastery"><h2><ShellIcon name="progress"/>Learning Mastery</h2><p className="ds-dash-caption">Regular practice · separate from exam readiness</p>{!result?<p role="status">{message}</p>:<div className="ds-mastery-list">{courses.map(course=>{const value=result.courses.find(c=>c.id===course.subject_id)!;return <Link to={'/progress?course='+course.subject_id} key={course.subject_id} data-course={course.subject_id}><span>{course.short}</span><div><strong>{value.index===null?'Not enough practice yet':`${value.index} / 100`}</strong><small>{value.covered}/{value.total} skills with reliable practice · Confidence: {value.confidence}</small></div><ShellIcon name="arrow" size={16}/></Link>;})}</div>}<p className="ds-dash-caption">Missing practice stays unknown. Reading and ungraded answers do not establish correctness.</p></section>
  </>;
}
