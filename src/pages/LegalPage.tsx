...
const effectiveDate='19 September 2026';
...
export function LegalPage({kind}:{kind:LegalKind}) {
  if(kind==='privacy')return <article className="ds-legal">
    ...
    <section><h2>Cookies and analytics</h2><p>Notedra uses PostHog in the European Union for limited product analytics. Only explicit events about feature use are sent, with a non-persistent anonymous identifier. Notedra does not send study answers, submitted code, free text, email addresses, display names, account or Supabase IDs, grades, detailed feedback, uploaded content, or full URLs. GeoIP processing, person profiles, identification, automatic page views, session replay, performance capture, and persistent analytics storage are disabled. The hosting and authentication providers may use strictly necessary security or session cookies to deliver the site, limit automated abuse, and keep you signed in. Browser storage is also used for the functional purposes described above. You can block analytics requests through browser or network controls and contact Notedra below with privacy questions.</p></section>
