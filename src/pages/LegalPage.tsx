...
export function LegalPage({kind}:{kind:LegalKind}) {
  if(kind==='privacy')return <article className="ds-legal">
    <header className="ds-page-heading"><p className="ds-eyebrow">LEGAL</p><h1>Privacy Policy</h1><p>Effective {effectiveDate}. This page explains what DelftStudy stores and why.</p></header>
    <section><h2>Information DelftStudy handles</h2><p>You can use core study features without an account. In anonymous mode, study records and preferences are stored in your browser. If you create an account, Supabase Auth handles your email address and login session, and DelftStudy stores your display name and synchronized learner snapshot under your account ID.</p></section>
    ...
    <section><h2>Local storage, cloud sync, and backups</h2><p>DelftStudy uses browser storage for account separation, appearance preferences, saved work, and anonymous progress. Signed-in learners can synchronize supported records with Supabase. You can export a JSON backup from Account &amp; sync. Importing a backup replaces the current local profile after an explicit confirmation and keeps a local recovery copy.</p></section>
    <section><h2>Cookies and analytics</h2><p>DelftStudy does not add analytics, advertising, or marketing trackers. The hosting and authentication providers may use strictly necessary security or session cookies to deliver the site, limit automated abuse, and keep you signed in. Browser storage is also used for the functional purposes described above.</p></section>
    <section><h2>Sharing and retention</h2><p>DelftStudy does not sell learner data. Supabase processes account and synchronized data, and the hosting provider processes technical request data needed to deliver and protect the service. Local records remain until you clear them or your browser data. Cloud account and learner records remain until they are deleted under the service’s operational process; no shorter retention period is promised here.</p></section>
    <section><h2>Your choices</h2><p>You can study anonymously, sign out, export your records, clear browser data, and choose whether to create an account. You may request access, correction, export, or deletion of account data through the contact method below. Identity verification may be required before an account request is completed.</p></section>
    ...
    <p className="ds-legal-back"><Link to="/">Return to DelftStudy</Link></p>
  </article>;
  return <article className="ds-legal">
    <header className="ds-page-heading"><p className="ds-eyebrow">LEGAL</p><h1>Terms of Use</h1><p>Effective {effectiveDate}. Please use DelftStudy responsibly.</p></header>
    <section><h2>Independent study support</h2><p>DelftStudy is an independent educational platform. It is not an official TU Delft service and does not represent university endorsement. Course names identify the material being studied.</p></section>
    ...
    <section><h2>Content and intellectual property</h2><p>DelftStudy respects the ownership of course sources and other referenced material. Access to the platform does not transfer rights in DelftStudy, source material, university marks, or third-party content. Use study material within the permissions that apply to it.</p></section>
