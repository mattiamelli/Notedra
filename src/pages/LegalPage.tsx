...
export function LegalPage({kind}:{kind:LegalKind}) {
  if(kind==='privacy')return <article className="ds-legal">
    ...
    <section><h2>Information Notedra handles</h2><p>You can use core study features without an account. In anonymous mode, study records and preferences are stored in your browser. If you create an account, Supabase Auth handles your email address and login session, and Notedra stores your display name and synchronized learner snapshot under your account ID.</p></section>
    ...
    <section><h2>Local storage, cloud sync, and backups</h2><p>Notedra uses browser storage for account separation, appearance preferences, saved work, and anonymous progress. Signed-in learners can synchronize supported records with Supabase. You can export a JSON backup from Account &amp; Settings. Importing a backup replaces the current local profile after an explicit confirmation and keeps a local recovery copy.</p></section>
