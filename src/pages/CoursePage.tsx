...
export function CoursePage({course}: {course: Course}) {
  ...
  return <>
    <PageHeading eyebrow={`${course.code} · ${course.short}`} title={course.name}><p>{topics.length} topics in the academic catalogue.</p>{['CSE1400_CO','CSE1300_RL'].includes(course.subject_id)&&<p>Follow the canonical topic order and check each topic’s prerequisites. Lessons, cards and tools are available for study; reading does not mark a topic complete.</p>}</PageHeading>
