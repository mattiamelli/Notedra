...
export function CoursePage({course}: {course: Course}) {
  ...
  return <>
    <PageHeading eyebrow={`${course.code} · ${course.short}`} title={course.name}><p>{topics.length} topics in the academic catalogue.</p>{course.subject_id==='CSE1400_CO'&&<p>Follow the canonical topic order and check each topic’s prerequisites. Lessons, cards and tools are available for study; reading does not mark a topic complete.</p>}</PageHeading>
