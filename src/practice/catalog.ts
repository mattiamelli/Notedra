...
export const allExercises: readonly PracticeExercise[] = freeze([...catalog, ...coDefinitions as COExercise[],...rlDefinitions as RLExercise[]]);
...
export function versionBinding(exercise: PracticeExercise): string {
  ...
  const fingerprint = ((isRL ? rlFingerprints : isCO ? coFingerprints : fingerprints) as Record<string, string>)[`${exercise.id}@${exercise.version}`];
  if (!fingerprint) throw new Error('Original exercise version unavailable.');
  return `${exercise.version}:sha256:${fingerprint}:grader:${isRL ? rlGraderLock.sha256 : isCO ? coGraderLock.sha256 : graderLock.sha256}`;
}
