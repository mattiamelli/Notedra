...
export function PracticePage() {
  ...
  return <><PageHeading eyebrow={t('practice.authored').toUpperCase()} title={t('practice.title')}><p>{t('practice.catalogDescription',{count:catalog.length})}</p></PageHeading>
    ...
    <section className="ds-practice-grid" aria-label={t('practice.title')}>{filtered.map(exercise=><article className="ds-practice-card" key={exercise.id}><p className="ds-practice-label">{courses.find(course=>course.subject_id===exercise.subjectId)!.code} · {t('practice.authored')}</p><h2>{lt(exercise.title)}</h2><p>{academicIndex.topics.find(topic=>topic.topic_id===exercise.topicId)!.name}</p><Link className="ds-button" to={exercisePath(exercise)}>{t('practice.openExercise')}<span className="sr-only">: {lt(exercise.title)}</span></Link></article>)}</section>
