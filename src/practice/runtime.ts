...
export function gradeResponse(exercise:PracticeExercise,answer:Answer){return isInteractive(exercise.task)?gradeInteractive(exercise as InteractiveExercise,answer):exercise.task.kind==='ip-fixed'?gradeIPResponse(exercise as IPExercise,answer):exercise.task.kind==='enrichment-exact'?gradeEnrichment(exercise as EnrichmentExercise,answer):exercise.task.kind==='rl-exact'?gradeRLResponse(exercise as RLExercise,answer):exercise.task.kind==='co-exact'?gradeCOResponse(exercise as COExercise,answer):legacyGrade(exercise as Exercise,answer);}
