...
export function gradeResponse(exercise:PracticeExercise,answer:Answer){return isInteractive(exercise.task)&&hasComplements(exercise.task)?gradeComplement(exercise as InteractiveExercise,answer):isInteractive(exercise.task)?gradeInteractive(exercise as InteractiveExercise,answer):exercise.task.kind==='ip-fixed'?gradeIPResponse(exercise as IPExercise,answer):exercise.task.kind==='enrichment-exact'?gradeEnrichment(exercise as EnrichmentExercise,answer):exercise.task.kind==='rl-exact'?gradeRLResponse(exercise as RLExercise,answer):exercise.task.kind==='co-exact'?gradeCOResponse(exercise as COExercise,answer):legacyGrade(exercise as Exercise,answer);}
export function validateResponse(task:InteractiveTask|Task|COTask|RLTask|EnrichmentTask|IPTask,answer:Answer){return isInteractive(task)&&hasComplements(task)?validateComplement(task,answer):isInteractive(task)?validateInteractive(task,answer):task.kind==='ip-fixed'?validateIPResponse(task,answer):task.kind==='enrichment-exact'?validateEnrichmentResponse(task,answer):task.kind==='rl-exact'?validateRLResponse(task,answer):task.kind==='co-exact'?validateCOResponse(task,answer):legacyValidate(task,answer);}
export function initialAnswer(exercise:PracticeExercise):Answer{if(isInteractive(exercise.task)||exercise.task.kind==='ip-fixed')return {kind:'choice',value:[]};return exercise.task.kind==='enrichment-exact'||exercise.task.kind==='co-exact'||exercise.task.kind==='rl-exact'?{kind:'text',value:''}:legacyInitial(exercise as Exercise);}
