...
function LessonSection({block,anchorIds}:{block:LessonBlock;anchorIds:string[]}){
 const {t}=useI18n();const takeaway=['important_rule','recap'].includes(block.kind)&&block.paragraphs.length>1?block.paragraphs.at(-1):null;const paragraphs=takeaway?block.paragraphs.slice(0,-1):block.paragraphs;
 return <article className={`ds-lesson-block ds-lesson-${block.kind}`} id={block.id} aria-labelledby={`${block.id}-title`}>{anchorIds.map(id=><span className="ds-learning-anchor" id={id} key={id} aria-hidden="true"/>)}<p className="ds-study-eyebrow">{t(kindKeys[block.kind])}</p><h3 id={`${block.id}-title`}>{block.title}</h3><div className="ds-lesson-copy">{paragraphs.map((p,i)=><p key={i}>{p}</p>)}</div>{block.code&&<pre aria-label={`Java example: ${block.title}`}><code>{block.code}</code></pre>}{block.table&&<div className="ds-study-table-wrap"><table><caption>{block.title} — reference table</caption><thead><tr>{block.table.headers.map(h=><th scope="col" key={h}>{h}</th>)}</tr></thead><tbody>{block.table.rows.map((r,i)=><tr key={i}>{r.map((v,j)=><td key={j}>{v}</td>)}</tr>)}</tbody></table></div>}{takeaway&&<aside className="ds-key-takeaway"><strong>{t('learning.keyTakeaway')}</strong><p>{takeaway}</p></aside>}<AuthoredSources item={block}/></article>;
}
export function LearnMode({topic,content}:{topic:StudyTopic;content?:Lesson}){
 ...
 return <section className="ds-study-reading"><header className="ds-learn-intro ds-lesson-block ds-lesson-introduction" id={introduction?.id}><p className="ds-study-eyebrow">{t('topic.learn')}</p><h2>{t('learning.learnTopic',{topic:topic.name})}</h2>{introduction&&<h3>{introduction.title}</h3>}{(introduction?.paragraphs??[topic.description,topic.relevance]).slice(0,2).map((p,i)=><p key={i}>{p}</p>)}<div className="ds-key-concepts"><strong>{t('learning.keyConcepts')}</strong><ul>{topic.subtopics.map(subtopic=><li key={subtopic.id}>{subtopic.name}</li>)}</ul></div>{introduction&&<AuthoredSources item={introduction}/>}</header>
 <nav aria-label={t('learning.lessonSections')} className="ds-lesson-outline"><strong>{t('learning.inThisLesson')}</strong><ol>{sections.map(b=><li key={b.id}><a href={`#${b.id}`}>{b.title}</a></li>)}</ol></nav>
...
export function FlashcardMode({topic,content}:{topic:StudyTopic;content?:Flashcard[]}){
 ...
 <p role="status" className="ds-card-position">Card {position+1} of {cards.length}</p><article className="ds-study-card" aria-labelledby="card-prompt"><span className="ds-study-eyebrow">Recall, then reveal</span><h3 id="card-prompt">{card.prompt}</h3><button className="ds-button ds-practice-primary" aria-expanded={revealed} aria-controls="card-answer" onClick={()=>setRevealed(v=>!v)}>{revealed?'Hide answer':'Reveal answer'}</button><div id="card-answer" hidden={!revealed}><h4>Answer</h4><p>{card.answer}</p></div></article>
