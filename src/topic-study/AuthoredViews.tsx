...
export function LearnMode({topic,content}:{topic:StudyTopic;content?:Lesson}){
 ...
 return <section className="ds-study-reading"><h2>Learn</h2><p className="ds-study-muted">Authored Notedra learning content · Version {lesson.version}. Explanations are not official TU Delft wording.</p><nav aria-label="Lesson sections" className="ds-lesson-outline"><ol>{lesson.blocks.map(b=><li key={b.id}><a href={`#${b.id}`}>{b.title}</a></li>)}</ol></nav>
