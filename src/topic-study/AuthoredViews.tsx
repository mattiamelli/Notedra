...
import type { AuthoredScope,StudyTopic } from './types';
...
export function LearnMode({topic}:{topic:StudyTopic}){
 const lesson=lessonFor(topic.id);
...
export function FlashcardMode({topic}:{topic:StudyTopic}){
 const cards=cardsFor(topic.id);const initial=cards.map(c=>c.id);
