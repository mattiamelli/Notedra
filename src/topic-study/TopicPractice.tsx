...
export function TopicPractice({topicId,hasStudyActivities=false,mode}:{topicId:string;hasStudyActivities?:boolean;mode?:'exam'}){
 const learning=useLearning();const exercises=catalog.filter(e=>e.topicId===topicId&&(!mode||('mode' in e&&e.mode===mode)));
 if(!exercises.length&&hasStudyActivities)return <section><h2>{mode==='exam'?'Exam-style challenges':'Practice'}</h2><p>No automatically graded exercises for this topic. Use the guided activities or specialized tool on this page.</p></section>;
 ...
 return <section><h2>{mode==='exam'?'Exam-style challenges':'Practice'}</h2><p>{exercises.length} authored exercises from the shared Practice catalog. Saved work opens in the same runner; these are not official exam questions.</p><div className="ds-practice-grid">{exercises.map(exercise=>{
