...
export function TopicOverview({topic}:{topic:StudyTopic}){
 const {lt}=useI18n();
 return <div className="ds-study-overview"><section className="ds-study-intro"><h2>Overview</h2><p>{lt(topic.description)}</p><h3>Prerequisite topics</h3><Prerequisites ids={topic.prerequisites}/><Sources ids={topic.sources} label="Source documents"/></section>
 <section aria-labelledby="topic-scope-heading"><h2 id="topic-scope-heading">Subtopics and skills</h2><div className="ds-study-scope">{topic.subtopics.map(sub=><article key={sub.id} id={sub.id} className="ds-study-subtopic"><h3>{lt(sub.name)}</h3><h4>Prerequisites</h4><Prerequisites ids={sub.prerequisites}/><ul className="ds-study-skills">{sub.skills.map(skill=><li key={skill.id} id={skill.id}><h4>{lt(skill.name)}</h4><p>{lt(skill.description)}</p>{skill.prerequisites.length>0&&<><p>Prerequisites</p><Prerequisites ids={skill.prerequisites}/></>}<Sources ids={skill.sources} label={`Sources for ${skill.name}`}/></li>)}</ul><Sources ids={sub.sources} label={`Sources for ${sub.name}`}/></article>)}</div></section></div>;
}
export function MentalMap({topic}:{topic:StudyTopic}){
 const {t,lt}=useI18n();const map=mapStructure(topic);const canvas=useRef<HTMLDivElement>(null);const [reversed,setReversed]=useState(false);const [focused,setFocused]=useState(false);const branches=reversed?[...topic.subtopics].reverse():topic.subtopics;
