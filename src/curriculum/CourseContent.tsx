...
export function CurriculumRevision({courseId,topicId}:{courseId:string;topicId:string}) {
  ...
  return <section className="ds-curriculum-revision"><h2>Authored revision</h2><p>Independent revision, not an official mock exam. These questions revisit the topic's practice material. Open reasoning uses self-review rubrics and does not establish exam readiness.</p>
    {selected.length>0&&<><h3>Mixed practice</h3><ol className="ds-curriculum-revision-list">{selected.map(item=><li key={item.id}><h4>{item.title}</h4><p>{item.prompt}</p>{exerciseLink(item.id)}</li>)}</ol></>}
    {reasoning.length>0&&<><h3>Open reasoning</h3><ol className="ds-curriculum-revision-list">{reasoning.map(item=><li key={item.id}><h4>{item.title}</h4><p>{item.prompt}</p>{exerciseLink(item.id)}<details><summary>Self-review rubric</summary><ul>{item.rubric?.map((criterion,index)=><li key={index}>{criterion}</li>)}</ul><p>{item.explanation}</p></details></li>)}</ol></>}
