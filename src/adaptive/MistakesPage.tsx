...
function MistakeCard({mistake:m,group}:{mistake:Mistake;group?:SkillEvidence}){
 ...
 return <article className="ds-mistake-card" aria-label={m.exercise.title}>
  ...
  <details><summary>Review the reasoning</summary><p>{m.explanation?.reasoning??m.exercise.explanation}</p><p>{m.explanation?.remember??m.exercise.rules}</p><p>Original definition and grader binding verified. <Link to={attemptPath(m.exercise,m.attempt.attemptId)}>Open immutable submission</Link></p></details>
...
export function MistakesPage({topicId}:{topicId?:string}){
 ...
 return <div className="ds-adaptive">
  ...
  {evidence.limited.length>0&&<details className="ds-historical"><summary>{evidence.limited.length} submissions with limited or unavailable historical evidence</summary><p>These are preserved, not counted as wrong. No current grader is substituted.</p>{evidence.limited.filter(({attempt})=>(!filter.topic||attempt.topicId===filter.topic)&&(!filter.course||attempt.subjectId===filter.course)).slice(0,20).map(({attempt,reason})=><div key={attempt.attemptId}><p>{attempt.templateRef??'Unscored saved answer'} · {reason}</p><pre>{attempt.answer.kind==='choice'?attempt.answer.value.join(', '):attempt.answer.value}</pre></div>)}<p>All original records remain available in your student backup.</p></details>}
 </div>;
}
