...
export function StudyPathPage({build=buildStudySession}:{build?:typeof buildStudySession}){
 ...
 function generate(){try{setPath(build(evidence,{minutes,subjectId,topicId:topicId||undefined}));setError('');}catch{setPath(null);setError(t('studyPath.errorKeep'));}}
 ...
 return <div className="ds-adaptive"><PageHeading title={t('studyPath.title')} eyebrow={t('studyPath.eyebrow').toUpperCase()}><p>{t('studyPath.description')}</p></PageHeading>
  ...
  {path===null?<EmptyState title={t(subjectId?'studyPath.ready':'studyPath.choose')}><p>{t(subjectId?'studyPath.buildHint':'studyPath.courseRequired')}</p></EmptyState>:<><p role="status">{t('studyPath.sessionSummary',{duration:studyTimeLabel(minutes,t),count:path.length,minutes:path.reduce((total,action)=>total+action.minutes,0)})}</p><ol className="ds-study-queue">{path.map(action=><li key={action.id}><article className="ds-path-card"><div className="ds-evidence-meta"><span>{courses.find(c=>c.subject_id===action.subjectId)!.short} · {actionKind(action.kind,t)}</span><span>{t('duration.short',{count:action.minutes})} · {t('duration.estimate')}</span></div><h2>{presentAction(action,t,lt).title}</h2><p>{academicIndex.topics.find(topic=>topic.topic_id===action.topicId)?.name??action.topicId}</p><p className="ds-path-reason"><strong>{t('studyPath.why')}</strong><br/>{presentAction(action,t,lt).reason}</p><Link className="ds-button" to={action.to}>{t('studyPath.openActivity')} →</Link></article></li>)}</ol><div className="ds-study-next"><Link className="ds-button" to={path[0].to}>{t('studyPath.start')} →</Link><button className="ds-button ds-button-secondary" onClick={generate}>{t('studyPath.rebuild')}</button></div></>}
  <details><summary>{t('studyPath.how')}</summary><p>{t('studyPath.howEvidence')}</p><p>{t('studyPath.howLonger')}</p><details><summary>{t('studyPath.technical')}</summary><p>{t('studyPath.technicalBody')}</p></details></details>
 </div>;
}
