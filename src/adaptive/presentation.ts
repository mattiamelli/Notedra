...
const structuredSuffixes=new Set(['focus','learn','practice','recall','review','interleave']);
function presentationId(id:string){
 const raw=id.startsWith('session:')?id.slice(8):id;
 if(!id.startsWith('session:'))return raw;
 const parts=raw.split(':');
 return structuredSuffixes.has(parts.at(-1)??'')?parts.slice(0,-1).join(':'):raw;
}
...
export function presentAction(action:Recommendation,t:T,lt:LT){
 ...
 const raw=presentationId(action.id);
 ...
 else if(raw.startsWith('subtopic:')){const sub=topicStudy.topics.flatMap(item=>item.subtopics).find(item=>item.id===raw.slice(9));if(sub){title=t('action.focus',{name:lt(sub.name)});reason=t('action.focusReason',{name:lt(sub.name),topic});}else{title=t('action.study',{topic});reason=t('action.learnReason',{topic});}}
