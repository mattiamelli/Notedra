...
const root=resolve('.');
prepareSitesStage(root,['dist/_headers','dist/index.html','dist/404.html','.openai/hosting.json','hosting/policy.json']);
...
const expectedFlag=process.argv.indexOf('--expected-source-sha'),expectedSourceSha=expectedFlag===-1?undefined:process.argv[expectedFlag+1];
if(expectedFlag!==-1&&!expectedSourceSha)throw Error('--expected-source-sha requires a value');
const releaseManifest=writeSitesPackage(root,worker,config,resolveSourceSha(root,expectedSourceSha));
console.log('Sites Worker artifact prepared', {documents:Object.keys(documents).length,assets:Object.keys(assets).length,workerBytes:Buffer.byteLength(worker),csp:policy.cspMode,sourceSha:releaseManifest.sourceSha,artifactSha256:releaseManifest.artifactSha256});
