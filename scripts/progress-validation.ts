...
export function validateMastery(){
 ...
 const now=Date.UTC(2026,8,10),empty=deriveProgress(emptyBackup(),now);assert.equal(empty.skills.length,topicStudy.topics.flatMap(topic=>topic.subtopics.flatMap(subtopic=>subtopic.skills)).length);assert.equal(empty.topics.length,topicStudy.topics.length);assert.equal(empty.courses.length,11);assert(empty.skills.every(s=>s.index===null&&s.confidence==='Insufficient'));
 assert.equal(empty.skills.filter(skill=>['CSE1400_CO','CSE1300_RL','CSE1100_IP'].includes(skill.courseId)).length,147);
 ...
 return {skills:empty.skills.length,topics:empty.topics.length,courses:empty.courses.length,schema:3,db:4,protectedFiles:Object.keys(lock).length};
}
export function validateReadiness(){
 ...
 assert.deepEqual(Object.fromEntries(r.filter(course=>['CSE1400_CO','CSE1300_RL','CSE1100_IP'].includes(course.courseId)).map(c=>[c.courseId,c.totalTopics])),{CSE1400_CO:13,CSE1300_RL:9,CSE1100_IP:20});assert.deepEqual(['Easy','Medium','Hard','Unknown'].map(d=>difficultyWeight(d as Observation['difficulty'])),[.9,1,1.1,1]);
 ...
 return {courses:r.length,requiredOpenMechanisms:Object.fromEntries(Object.entries(requiredOpen).map(([k,v])=>[k,v.length])),unknown:true};
}
