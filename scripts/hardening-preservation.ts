...
const changes=lock as Record<string,{before:string;after:string;original?:string}>;
export function hardeningHash(file:string,previous:string):string {
 ...
 assert.ok(previous===entry.before||previous===entry.original,'Unrecognized hardening baseline: '+file);return entry.after;
}
...
export function beforeHardening(file:string,data:Buffer):Buffer {
 ...
 assert.equal(createHash('sha256').update(original).digest('hex'),entry.original??entry.before);return original;
}
