...
export function validateMistakes(){
 assert.equal(STUDENT_SCHEMA_VERSION,2);assert.equal(STUDENT_DB_VERSION,2);
 const {reviews:_reviews,...base}=emptyBackup();const legacy={...base,schemaVersion:1};assert.deepEqual(migrateBackup(legacy),emptyBackup());
 for(const [file,value] of Object.entries(lock))assert.equal(createHash('sha256').update(readFileSync(file)).digest('hex'),value.after,'Storage migration source drift: '+file);
 ...
 return {schema:2,db:2,atomicItems:allExercises.length};
}
