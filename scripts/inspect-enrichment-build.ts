...
const files=readdirSync('dist',{recursive:true,withFileTypes:true}).filter(f=>f.isFile()).map(f=>{const file=relative('dist',f.parentPath+'/'+f.name);return {file,prefix:readFileSync('dist/'+file).subarray(0,16).toString()};});
