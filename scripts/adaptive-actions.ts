...
export function buildAdaptiveActions():Action[]{
 ...
 function add(row:SourceAction,kind:Action['kind']){
  ...
  result.push({id:row.id,title:row.title??row.name!,topicId:row.topicId,subjectId:topic.subjectId,skillIds:row.skillIds,kind,minutes:row.minutes??10,durationSource:row.minutes?'Authored practice duration':'Notedra estimate',to:kind==='coding'?`${route}/practice?task=${encodeURIComponent(row.id)}`:row.id==='assembly'?ASSEMBLY_TOOL_PATH:`${route}/practice`});
 }
