...
type Activity={id:string;topicId:string;skillIds:string[];sourceIds:string[];title?:string;name?:string;fields?:unknown[];assumptions?:string};
...
export function buildExerciseCoverage(exercises:readonly PracticeExercise[]=allExercises){
 ...
 for(const e of exercises){
  ...
  rows.push({id:e.id,...s,title:e.title,before:e.task.kind+variant,after:policy.renderer+variant,source:[`${e.source.documentId}: ${e.source.filename} (${e.source.precision}; ${e.source.locator})`],practice:'Focused fixed task; optional topic preparation; immediate authored feedback; retry/new unseen same-topic item.',exam:exam.length?'Exact version reused inside timed mixed blueprint; no preparation panel or feedback until submission. '+exam.map(i=>i.id).join(', '):'No fixed exam reuse; see separate topic exam rows or explicit gap.',validator:policy.validator+' / '+versionBinding(e),feedback:policy.feedback+'; authored explanation/rules; exact authored misconception only where available.',tests:'exercise-architecture; practice-grading; practice-ui; '+(s.course==='CSE1400_CO'?'co-reference':s.course==='CSE1300_RL'?'rl-practice':'ip-practice')+'; enrichment',fallback:e.task.kind==='enrichment-exact'?'Legacy wrong-arity saved draft retains raw text; no truncation. Correct-arity answers use ordered fields.':'Retain bounded authored format: a unique declared result is safely checkable; this does not grade the whole skill.'});
 }
 function activity(a:Activity,family:string,file:string){
  const tool=family==='workspace',code=family==='code task';
  rows.push({id:a.id,...scope(a.topicId,a.skillIds),title:a.title??a.name??a.id,before:family,after:family+(a.fields?' / labelled reasoning fields':''),source:sources(a.sourceIds),practice:tool?'Explore the existing bounded domain model.':code?'Write local Java notes; inspect requirements and authored reference.':'Work through source-grounded prompt and rubric; reveal/reference where available.',exam:code?'Integrated 60-minute assignments also exposed in programming exam practice; session bank has separately authored integrated rubric tasks.':'Not automatically converted into an exam question; separate exam bank coverage below.',validator:'Unscored; '+(tool?'existing bounded tool semantics, not skill grading':code?'learner Java is never executed':'human/self rubric review'),feedback:tool?'Visible model state and explicit assumptions':code?'Requirements, rubric and authored Java reference':'Authored component criteria/reference; no inferred correctness',tests:code?'ip-assignment-references; ip-ui; Java verification':family==='workspace'?'co-models; rl-models; assembly-maintenance':file.includes('/enrichment/')?'enrichment; enrichment-ui; browser acceptance':`${file.includes('/co/')?'co':file.includes('/rl/')?'rl':'ip'}-content; browser acceptance`,fallback:a.assumptions??(code?'Open implementation admits multiple valid solutions; no regex or keyword score.':'Open reasoning/construction cannot be objectively established by the existing bounded graders. Rubric is intentional, not missing automated correctness.')});
 }
