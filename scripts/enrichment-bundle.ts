...
export function validateEnrichmentBundle(chunks:BundleChunk[],files:{file:string;prefix:string;content?:string}[]){
 ...
 for(const file of files){
  const releaseRobots=isReleaseRobots(file);
  if((/\.(?:pdf|zip|txt|pages\.json)$/i.test(file.file)&&!releaseRobots)||/Handoff|source-inventory|enrichment-baseline/i.test(file.file)||file.prefix.startsWith('%PDF-'))throw new Error('Raw source emitted: '+file.file);
 }
}
