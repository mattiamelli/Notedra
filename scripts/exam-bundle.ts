...
export function validateExamBundle(chunks:IPBundleChunk[],files:IPBundleArtifact[]){
 ...
 for(const f of files)if(/\.(pdf|zip|txt)$/i.test(f.file)||/Handoff|exam-source-review|exam-content-lock/.test(f.file)||/^(%PDF-|PK\x03\x04)/.test(f.prefix))throw Error('Raw source artifact bundled');
