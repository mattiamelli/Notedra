-- Step 12: learner-owned snapshots and permanent idempotency receipts, cloud schema 1.
-- No canonical content, navigation, scores derived by Progress, or authentication secrets.
begin;
...
create table public.learner_sync_operations (
...
alter table public.learner_snapshots force row level security;
alter table public.learner_sync_operations enable row level security;
alter table public.learner_sync_operations force row level security;

create policy snapshot_select on public.learner_snapshots for select to authenticated using ((select auth.uid()) = user_id);
create policy snapshot_insert on public.learner_snapshots for insert to authenticated with check ((select auth.uid()) = user_id);
create policy snapshot_update on public.learner_snapshots for update to authenticated using ((select auth.uid()) = user_id) with check ((select auth.uid()) = user_id);
create policy snapshot_delete on public.learner_snapshots for delete to authenticated using ((select auth.uid()) = user_id);
create policy operation_select on public.learner_sync_operations for select to authenticated using ((select auth.uid()) = user_id);
create policy operation_insert on public.learner_sync_operations for insert to authenticated with check ((select auth.uid()) = user_id);
create policy operation_update on public.learner_sync_operations for update to authenticated using ((select auth.uid()) = user_id) with check ((select auth.uid()) = user_id);
create policy operation_delete on public.learner_sync_operations for delete to authenticated using ((select auth.uid()) = user_id);

-- All writes go through the owner-bound, revision-checked function. There is no deletion API.
revoke all on public.learner_snapshots, public.learner_sync_operations from public, anon, authenticated;
grant select on public.learner_snapshots, public.learner_sync_operations to authenticated;

create function public.sync_learner_snapshot(expected_revision bigint, operation uuid, learner_payload jsonb)
...
begin
  ...
  if coalesce(existing.revision,0) <> expected_revision then raise exception 'Stale cloud revision' using errcode = '40001'; end if;
