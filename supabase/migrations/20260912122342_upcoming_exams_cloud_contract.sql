-- Maintenance Patch 2: extend the learner snapshot with optional upcoming exams.
begin;
...
begin
  ...
  if learner_payload is null or jsonb_typeof(learner_payload) <> 'object' then
  ...
  if operation is null or expected_revision is null or expected_revision < 0 or expected_revision >= 9007199254740991
    ...
    or (select array_agg(key order by key) from jsonb_object_keys(learner_payload - 'upcomingExams') as key)
  ...
  -- Patch 2: optional planning data. Do not normalize the request: receipts hash exact payloads.
  if learner_payload ? 'upcomingExams' then
    if jsonb_typeof(learner_payload->'upcomingExams') is distinct from 'array' then
      raise exception 'Invalid upcoming exam collection' using errcode = '22023';
    end if;
    if jsonb_array_length(learner_payload->'upcomingExams') > 50 then
      raise exception 'Upcoming exam capacity exceeded' using errcode = '22023';
    end if;
    identities := array[]::text[];
    for new_record in select value from jsonb_array_elements(learner_payload->'upcomingExams') loop
      if jsonb_typeof(new_record) is distinct from 'object' then
        raise exception 'Invalid upcoming exam object' using errcode = '22023';
      end if;
      if (select array_agg(key order by key) from jsonb_object_keys(new_record) as key)
          is distinct from array['examDate','id','name']::text[]
        or jsonb_typeof(new_record->'id') is distinct from 'string'
        or jsonb_typeof(new_record->'name') is distinct from 'string'
        or jsonb_typeof(new_record->'examDate') is distinct from 'string'
        then raise exception 'Invalid upcoming exam fields' using errcode = '22023'; end if;
      record_id := new_record->>'id';
      -- Match JavaScript string.length (UTF-16 units), including supplementary characters.
      if length(record_id) = 0 or record_id = any(identities)
        or (select sum(case when ascii(c) > 65535 then 2 else 1 end)
              from regexp_split_to_table(record_id,'') as c) > 100
        or length(btrim(new_record->>'name', U&'\0009\000A\000B\000C\000D\0020\00A0\1680\2000\2001\2002\2003\2004\2005\2006\2007\2008\2009\200A\2028\2029\202F\205F\3000\FEFF')) = 0
        or (select sum(case when ascii(c) > 65535 then 2 else 1 end)
              from regexp_split_to_table(new_record->>'name','') as c) > 120
        then raise exception 'Invalid upcoming exam identity/name' using errcode = '22023'; end if;
      identities := array_append(identities,record_id);
      if (new_record->>'examDate') !~ '^[0-9]{4}-[0-9]{2}-[0-9]{2}$'
        or left(new_record->>'examDate',4) = '0000'
        then raise exception 'Invalid upcoming exam date' using errcode = '22023'; end if;
      begin
        perform (new_record->>'examDate')::date;
      exception when invalid_datetime_format or datetime_field_overflow then
        raise exception 'Invalid upcoming exam date' using errcode = '22023';
      end;
    end loop;
  end if;
